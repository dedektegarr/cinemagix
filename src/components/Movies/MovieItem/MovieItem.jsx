import { Link } from "react-router-dom";
import { formatDate } from "../../../utils/format-date";
import Card from "../../utils/Card";
import Button from "../../UI/Button/Button";
import useAddToFavorite from "../../../hooks/useAddToFavorite";

const MovieItem = ({ movie, className }) => {
  const { handleAddToFavorite, isFavorite } = useAddToFavorite(movie);

  let ratingColor;
  if (movie.vote_average < 5) {
    ratingColor = "border-red-700";
  } else if (movie.vote_average < 7) {
    ratingColor = "border-yellow-700";
  } else {
    ratingColor = "border-green-700";
  }

  return (
    <Card className={`group relative ${className}`}>
      <Card.Image
        src={`https://media.themoviedb.org/t/p/w300_and_h450_bestv2${movie.poster_path}`}
        alt="Movie Poster"
      />

      <Link to={`/movie/${movie.id}`}>
        <Card.Text
          title={movie.title || movie.name}
          subTitle={formatDate(movie.release_date || movie.first_air_date)}
        />
      </Link>

      <div className="absolute top-0 left-0 z-10 flex items-center justify-between w-full py-1 px-2">
        <div
          className={`bg-black/80 rounded-full p-2 text-xs border-2 ${ratingColor}`}
        >
          {movie.vote_average.toFixed(1)}
        </div>

        <Button
          onClick={handleAddToFavorite}
          className="translate-x-2"
          icon={
            isFavorite ? (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                height="24"
                viewBox="0 -960 960 960"
                width="24"
                fill="red"
              >
                <path d="m480-120-58-52q-101-91-167-157T150-447.5Q111-500 95.5-544T80-634q0-94 63-157t157-63q52 0 99 22t81 62q34-40 81-62t99-22q94 0 157 63t63 157q0 46-15.5 90T810-447.5Q771-395 705-329T538-172l-58 52Z" />
              </svg>
            ) : (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                height="24"
                viewBox="0 -960 960 960"
                width="24"
                fill="white"
              >
                <path d="m480-120-58-52q-101-91-167-157T150-447.5Q111-500 95.5-544T80-634q0-94 63-157t157-63q52 0 99 22t81 62q34-40 81-62t99-22q94 0 157 63t63 157q0 46-15.5 90T810-447.5Q771-395 705-329T538-172l-58 52Zm0-108q96-86 158-147.5t98-107q36-45.5 50-81t14-70.5q0-60-40-100t-100-40q-47 0-87 26.5T518-680h-76q-15-41-55-67.5T300-774q-60 0-100 40t-40 100q0 35 14 70.5t50 81q36 45.5 98 107T480-228Zm0-273Z" />
              </svg>
            )
          }
        />
      </div>
    </Card>
  );
};

export default MovieItem;
